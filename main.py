...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            x = angles[0] * 360
            y = angles[1] * 360
            p1 = face_landmarks.landmark[127]  # cheek
            p2 = face_landmarks.landmark[356]  # cheek
            vx = np.array([p2.x - p1.x, p2.y - p1.y])
            vx = vx / np.linalg.norm(vx)
            
            z = np.arccos(vx[0]) * 180 / np.pi
            if p1.y > p2.y:
                z = -z
            ...
            range_mouth = math.sqrt(math.pow(face_landmarks.landmark[16].x - face_landmarks.landmark[11].x, 2) + math.pow(face_landmarks.landmark[16].y - face_landmarks.landmark[11].y, 2))
