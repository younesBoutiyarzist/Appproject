...
class Moustache(pygame.sprite.Sprite):
    ...
    def move_rotation_p(self, speed=1, x=None,y=None):
        self.angle += speed
        xt = self.rect.x + (self.image.get_width()/2)
        yt = self.rect.y + (self.image.get_height()/2)
        self.image = pygame.transform.rotate(self.origine, self.angle)
        self.rect = self.image.get_rect(center = self.image.get_rect(center = (xt, yt)).center)
